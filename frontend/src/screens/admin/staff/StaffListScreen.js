...
export default function StaffListScreen({ navigation }) {
  ...
  const departments = [
    'All', 'Administration', 'Accounts', 'Operations', 'Customer Service', 'HR', 'IT Support'
  ];
  ...
  const fetchStaff = useCallback(async () => {
    try {
      ...
      if (filter !== 'All') params.department = filter;
      ...
      <View style={styles.flex1}>
        ...
        <View style={styles.tagsRow}>
          <View style={styles.tag}>
            <Text style={styles.tagText}>{item.department || 'N/A'}</Text>
          </View>
