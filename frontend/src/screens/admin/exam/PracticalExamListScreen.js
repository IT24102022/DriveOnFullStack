...
import { getPracticalExams } from '../../../services/examApi';
...
export default function PracticalExamListScreen({ navigation }) {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (filterCategory) params.vehicleCategory = filterCategory;
      
      const response = await getPracticalExams(params);
      setExams(response.data);
      setFilteredExams(response.data);
    } catch (error) {
      Alert.alert('Error', 'Could not load practical exams');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    ...
    if (search) {
      filtered = filtered.filter(exam =>
        exam.vehicleCategory.toLowerCase().includes(search.toLowerCase()) ||
        exam.trialLocation.toLowerCase().includes(search.toLowerCase())
      );
    }
  ...
  const renderExamCard = (exam) => {
    ...
    const getCategoryColor = (category) => {
      switch (category) {
        case 'Light': return COLORS.brandOrange;
        case 'Heavy': return COLORS.purple;
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('ExamDetails', { examType: 'practical', examId: exam._id })}
      >
        <View style={styles.examHeader}>
          <View style={styles.examInfo}>
            <Text style={styles.examTitle}>{exam.vehicleCategory} Practical Exam</Text>
        ...
        <View style={styles.examDetails}>
          <View style={styles.examDetailRow}>
            ...
            <Text style={styles.examDetailText}>{exam.trialLocation}</Text>
          </View>
          <View style={styles.examDetailRow}>
            <Ionicons name="car-outline" size={16} color={getCategoryColor(exam.vehicleCategory)} />
            <Text style={[styles.examDetailText, { color: getCategoryColor(exam.vehicleCategory) }]}>
              Category: {exam.vehicleCategory}
            </Text>
          </View>
        </View>
